...
- **`/mkesindo/produksi`** — modul dashboard (desktop). Masuk ke `NAV_ITEMS` sidebar seperti modul lain, tapi digerbang oleh flag `isProduksi` langsung (bukan sistem `ModuleKey`/`PermissionMap` granular seperti pnl/sales), konsisten dengan pola Driver/Satpam.
- **`/mkesindo/produksi-app`** — aplikasi mobile, gerbang sama (`isProduksi`), struktur tab (keep-alive shell) seperti `driver-app`.
