  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      try {
        ...
        // Extremely unlikely (the controller is brand new here), but guard
        // for consistency with the other enqueue call sites below.
        console.warn("Notification stream initial enqueue failed:", err);
      }
      ...
      heartbeat = setInterval(() => {
        try {
          ...
          // A failing enqueue here means this connection is genuinely dead,
          // not a one-off hiccup — keep retrying every 15s would just
          // rethrow (and get caught) forever until cancel() eventually
          // fires. Stop this connection's own heartbeat now; cancel() will
          // still run separately once the platform detects the disconnect
          // and will handle unregistering the event-bus listener.
          console.warn("Notification stream heartbeat enqueue failed for a stale connection:", err);
