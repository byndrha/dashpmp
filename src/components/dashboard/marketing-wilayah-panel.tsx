...
// Accounting-and-above admin panel for assigning which Wilayah/Kecamatan
// each Marketing is responsible for — visible only when canApprove (same
// gate as Setujui/Tolak on Pengajuan), since this is what determines whose
// responsibility a Mitra falls under everywhere else in the app.
export function MarketingWilayahPanel({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-display">Cakupan Wilayah Marketing</CardTitle>
        <CardDescription>
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex flex-wrap items-end gap-2 rounded-lg border bg-secondary/30 p-3">
          <div className="flex w-48 flex-col gap-1">
            <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Marketing</span>
          ...
          <div className="flex w-48 flex-col gap-1">
            <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Wilayah</span>
            <WilayahSelect value={wilayah} onChange={handleWilayahChange} />
          </div>
          <div className="flex w-48 flex-col gap-1">
            <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Kecamatan</span>
            {seluruhWilayah ? (
            ...
            <TableBody>
              {assignments.map((a) => (
                <TableRow key={a.MarketingWilayahID}>
                  ...
                  <TableCell>
                    {a.Kecamatan ?? <Badge variant="outline">Seluruh Wilayah</Badge>}
                  </TableCell>
                  <TableCell>
                    <Button
                      type="button"
