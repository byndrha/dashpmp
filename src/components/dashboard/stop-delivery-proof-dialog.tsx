...
import { JualUlangReturDialog } from "@/components/dashboard/jual-ulang-retur-dialog";
...
export function StopDeliveryProofDialog({
  ...
  // The open Jadwal's own JadwalID (RouteValidationDialog's own `jadwalId`,
  // derived from its `jadwal` prop) — not on DriverStopRow itself, so it's
  // threaded through as its own prop. Feeds JualUlangReturDialog's
  // DALAM_RUTE candidate list (getJadwalDetailAction(jadwalId)) and the
  // LUAR_RUTE/RETAIL jalur actions, which both record which Jadwal the
  // resale happened on. Nullable purely to mirror the caller's own
  // `jadwal?.JadwalID ?? null` — in practice this popup can only be opened
  // from a stop row that belongs to a currently-open (non-null) Jadwal.
  jadwalId: number | null;
  onOpenChange: (open: boolean) => void;
}) {
  const [proof, setProof] = useState<StopDeliveryProof | null>(null);
  const [loading, setLoading] = useState(false);
  // The retur item (per StopDeliveryProofItem) whose "Jual ke Mitra Lain"
  // button was clicked — null closes JualUlangReturDialog, same
  // open-via-prop convention this dialog itself uses for `detail`.
  const [jualUlangTarget, setJualUlangTarget] = useState<{ stopDeliveryItemId: number; itemName: string } | null>(
    null
  );

  useEffect(() => {
    // A Jual Ulang popup left open against the previously-shown stop must
    // not stay open against whichever stop (or none) is opened next.
  ...
  return (
    <>
                        ...
                        {item.kondisiRetur === "BAIK" && item.qtyRetur > 0 && (
                          <div className="mt-2 flex items-center justify-between gap-2 rounded-md bg-emerald-500/5 p-2">
                            ...
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                setJualUlangTarget({ stopDeliveryItemId: item.stopDeliveryItemId, itemName: item.name })
                              }
                            >
                              Jual ke Mitra Lain
                            </Button>
      ...
      <JualUlangReturDialog
        jadwalId={jadwalId}
        originJadwalDetailId={detail?.JadwalDetailID ?? null}
        target={jualUlangTarget}
        onOpenChange={(open) => !open && setJualUlangTarget(null)}
        onDone={() => setJualUlangTarget(null)}
      />
    </>
  );
}
