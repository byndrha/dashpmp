  ...
  callbacks: {
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.bolehGenerateKodeAmbilAlih = token.bolehGenerateKodeAmbilAlih as boolean;
        session.user.canAksesInventaris = token.canAksesInventaris as boolean;
