...
// Separate derived key from getKey()'s "perusahaan-db-credential:" and
// getGDriveKey()'s "gdrive-refresh-token:" prefixes — a leaked GPS provider
// credential (Hino Connect / SoloFleet) must not also unlock DB or Google
// Drive credentials, and vice versa (same principle as this file's existing
// getKey() comment).
function getGpsCredentialKey(): Buffer {
  const secret = process.env.AUTH_SECRET;
  if (!secret) throw new Error("AUTH_SECRET is not configured — cannot encrypt/decrypt stored secrets");
  return createHash("sha256").update(`gps-kendaraan-credential:${secret}`).digest();
}

export function encryptGpsCredential(plaintext: string): string {
  const key = getGpsCredentialKey();
  const iv = randomBytes(IV_LENGTH);
  const cipher = createCipheriv(ALGORITHM, key, iv);
  const encrypted = Buffer.concat([cipher.update(plaintext, "utf8"), cipher.final()]);
  const authTag = cipher.getAuthTag();
...
export function decryptGpsCredential(ciphertext: string): string {
  const key = getGpsCredentialKey();
  const buf = Buffer.from(ciphertext, "base64url");
  const iv = buf.subarray(0, IV_LENGTH);
  const authTag = buf.subarray(IV_LENGTH, IV_LENGTH + AUTH_TAG_LENGTH);
